...
const SamsungEvents = require('./SamsungEvents')
...
const buildPairingStepUri = (config, device, step) => {
  ...
  return `http://${config.ip}:${config.pinPort}${path}`
}
...
const _step1HelloServer = (config, device, pin) => {
  ...
  const serverHello = Pairing.generateServerHello(config.userId, pin)
...
const _step2AckServer = (config, device, requestId) => {
  ...
  const serverAck = Pairing.generateServerAcknowledge()
...
_verifyHelloAuthData = (authData) => {
  ...
  if (Pairing.parseClientHello(authData.GeneratorClientHello) !== 0) {
...
_verifyAckAuthData = (authData) => {
  ...
  const clientAck = Pairing.parseClientAcknowledge(authData.ClientAckMsg)
...
class SamsungTvPairing {
  ...
  requestPin() {
    return fetch(`http://${this.config.ip}:${this.config.pinPort}/ws/apps/CloudPINPage`,
      {
        method: 'POST',
        mode: 'no-cors',
        cache: 'default',
  ...
  confirmPin(pin) {
    return new Promise((resolve) => {
      ...
      this.eventEmitter.emit(SamsungEvents.PAIRED, this.identity)
    ...
    return _step1HelloServer(this.config, this.device, pin)
      ...
      .then(identity => {
        ...
        this.eventEmitter.emit(SamsungEvents.PAIRED, identity)
        return identity
      })
  }
  ...
  hidePinConfirmation() {
    ...
    return fetch(`http://${this.config.ip}:${this.config.pinPort}/ws/apps/CloudPINPage/run`, {
      method: 'DELETE'
    })
  }
